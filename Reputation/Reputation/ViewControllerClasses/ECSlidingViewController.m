...
@synthesize internalNWSelection = _internalNWSelection;
- (void)setTopViewController:(UIViewController *)theTopViewController
{
    _internalNWSelection = 1;
...
- (void)updateTopViewHorizontalCenterWithRecognizer:(UIPanGestureRecognizer *)recognizer
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"RSIGN_KEYBOARD" object:nil];
  CGPoint currentTouchPoint     = [recognizer locationInView:self.view];
    
    
  CGFloat currentTouchPositionX = currentTouchPoint.x;
    
    if (currentTouchPoint.x < 55 && currentTouchPoint.y < 55) {
        recognizer.enabled = FALSE;
        recognizer.enabled = TRUE;
    }
    else if (currentTouchPoint.x > 265 && currentTouchPoint.y < 55) {
        recognizer.enabled = FALSE;
        recognizer.enabled = TRUE;
    }
